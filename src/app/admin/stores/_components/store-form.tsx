...
export function StoreForm({ store }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    country: store?.address.country || "Sénégal",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const storeData = {
      ...
      // Null override fields
      logoUrl: store?.logoUrl ?? null,
      ninea: store?.ninea ?? null,
      rccm: store?.rccm ?? null,
      taxId: store?.taxId ?? null,
      proformaPrefix: store?.proformaPrefix ?? null,
      invoicePrefix: store?.invoicePrefix ?? null,
      depositPrefix: store?.depositPrefix ?? null,
      creditNotePrefix: store?.creditNotePrefix ?? null,
      orderPrefix: store?.orderPrefix ?? null,
      defaultPaymentTerms: store?.defaultPaymentTerms ?? null,
      defaultProformaValidity: store?.defaultProformaValidity ?? null,
      defaultInvoiceTemplate: store?.defaultInvoiceTemplate ?? null,
      defaultInvoiceNotes: store?.defaultInvoiceNotes ?? null,
    };
